...
def all_in_one():
    ...
    if len(candidates) == 0:
        ...
        torrent_category = (
            ...
            if candidates[0].category == candidates[1].category
            else None
        )
        id_and_location_tuple = (
            qbit.extract_id_and_path(candidates[0].name, torrent_category)
            if (
        ...
        id_and_location_tuple = qbit.extract_id_and_path(
            candidates[0].name, torrent_category
        )
    ...
    for candidate in candidates:
        ...
        if original_location is None:
            ...
            raise ValueError(
                f"All candidates must have the same original location, original_location is {original_location} but candidate {id_and_location_tuple[0]} has {id_and_location_tuple[1]}"
            )

    qbit.save_structure(
        original_location=original_location,
        save_file="struc.json",
    ...
    qbit.move_torrent(
        torrent_hashes=torrent_hashes,
        new_location=f"/megafarm/torrents/{torrent_category}",
    )
    general_original_location = f"/data/media/{torrent_category}"
    qbit.recreate_structure(
        original_location=general_original_location,
        new_location=f"/megafarm/media/{torrent_category}",
        save_file="struc.json",
        torrent_hashes=torrent_hashes,
    )
    qbit.let_starr_know(
        category=torrent_category,
        media_id=id_and_location_tuple[0],
        new_location=f"/megafarm/media/{torrent_category}",
    )
    qbit.close()
